...
"""
mutable struct TrunkSolver{T, V <: AbstractVector{T}, Op <: AbstractLinearOperator{T}}
  x::V
  xt::V
  gx::V
  gt::V
...
function TrunkSolver(
  ...
  Hs = V(undef, nvar)
  subsolver = CgSolver(nvar, nvar, V)
  H = hess_op!(nlp, x, Hs)
  Op = typeof(H)
  tr = TrustRegion(gt, one(T))
  return TrunkSolver{T, V, Op}(x, xt, gx, gt, gn, Hs, subsolver, H, tr)
end
...
function solve!(
  ...
  while !(solved || tired || stalled)
    ...
    s, cg_stats = cg!(subsolver, H, ∇f, atol = atol, rtol = cgtol, radius = tr.radius, itmax = max(2 * n, 50), verbose=0)
